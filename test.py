...
allowed_baseband_filter = pyhackrf.pyhackrf_compute_baseband_filter_bw_round_down_lt(
    baseband_filter
...
sdr.pyhackrf_set_antenna_enable(
    False
)  # It seems this setting enables or disables power supply to the antenna port. False by default. the firmware auto-disables this after returning to IDLE mode
...
print(
    f"center_freq: {center_freq} sample_rate: {sample_rate} baseband_filter: {allowed_baseband_filter}"
)
...
def rx_callback(
    device, buffer, buffer_length, valid_length
...
# Save figures to files instead of displaying them
plt.figure(0)
